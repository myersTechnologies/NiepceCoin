...
#if defined(NDEBUG)
# error "Niepcecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1fc264462243451d928b999b952165dc811fd2cd8dc45a162b5581f4c0ddfa2b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Niepcecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Niepcecoin Signed Message:\n";
...
{
    // Niepcecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Niepcecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Niepcecoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Niepcecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Niepcecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Niepcecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xfb7eb033e3c3224d6eafc82182c68a49784e114cf356bf4abd00580223d645c7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BBC News 26/04/2022 Niepcecoin was created!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041dde1802beb7ece7224ead7d05f2c7901ada8ae6a3727533ab749b15db92acad3be298c2fb662bee7fc7115974abd8030ce4f937ed375b8efc99068cf7a324bb") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1651009233;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084668583;

        if (fTestNet)
        {
            block.nTime    = 1651009215;
            block.nNonce   = 386922139;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x267c718a9825b16edeacd40f34f6052d31b1cdf13273b74c18d7bc82f97b1064"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x4e, 0x43, 0x53, 0x43 }; // Niepcecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NiepcecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NiepcecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NiepcecoinMiner(CWallet *pwallet)
{
    printf("NiepcecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("niepcecoin-miner");
    ...
    try { loop {
        ...
        printf("Running NiepcecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NiepcecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NiepcecoinMiner, pwallet));
}
